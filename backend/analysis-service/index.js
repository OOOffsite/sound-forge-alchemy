...
// Constants
const AUDIO_DATA_PATH = process.env.AUDIO_DATA_PATH || "http://localhost:3000/audio_data";
  ...
  try {
    ...
    analyzer.on("close", async (code) => {
      ...
      if (code === 0) {
        ...
        jobData.error = `Analyzer exited with code ${code || 'unknown'}: ${stderrData || 'Process was likely killed due to memory constraints'}`;
        jobData.completedAt = new Date().toISOString();
