  ...
  try {
    ...
    if (!trackId || (!spotifyUrl && !trackInfo)) {
      return res
        .status(400)
        .json({
          error: "Track ID and either Spotify URL or track info are required",
        });
    }
...
app.get("/track/:trackId", async (req, res) => {
  try {
    ...
    if (!jobIds || jobIds.length === 0) {
      return res
        .status(404)
        .json({ error: "No download jobs found for this track" });
    }
...
async function downloadTrack(jobId, trackId, spotifyUrl, trackInfo, trackDir) {
  try {
    ...
    spotdl.stderr.on("data", (data) => {
      ...
      if (
        str.includes("AudioProviderError") ||
        (str.includes("ERROR:") && !str.includes("INFO:"))
      ) {
      ...
      try {
        if (code === 0) {
          ...
          if (mp3Files.length > 0) {
            ...
            logger.info(
              `Download completed successfully for trackId: ${trackId}, jobId: ${jobId}`
            );
          } else {
