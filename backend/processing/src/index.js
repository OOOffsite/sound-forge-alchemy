...
const WEBSOCKET_SERVICE_URL =
  process.env.WEBSOCKET_SERVICE_URL || "http://websocket-service:3006";
    ...
    try {
      ...
      if (jobData.status === "completed" && jobData.outputPath) {
        ...
        if (autoProcess === "true") {
          ...
          const separationOptions = JSON.parse(
            (await redis.get(`track:${trackId}:separationOptions`)) || "{}"
          );
...
app.post("/models/download", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to download model", details: error.message });
  }
});
    ...
    if (!jobIds || jobIds.length === 0) {
      ...
      if (fs.existsSync(originalFile)) {
        ...
        return res
          .status(404)
          .json({
            error: "No processing jobs or audio file found for this track",
          });
      }
    }
      ...
      const stemFiles = fs
        ...
        .map((file) => {
          ...
          if (
            ...
            file.toLowerCase().includes("vocals")
          ) {
            stemType = "vocals";
          } else if (
            file.toLowerCase().includes("drum") ||
            file.toLowerCase().includes("drums")
          ) {
            stemType = "drums";
...
async function createProcessingJob(trackId, inputPath, options = {}) {
  ...
  if (options.autoProcess !== undefined) {
    await redis.set(
      ...
      options.autoProcess ? "true" : "false"
    );
  }
    ...
    demucs.on("close", async (code) => {
      try {
        if (code === 0) {
          ...
          const demucsOutputDir = path.join(
            path.dirname(outputPath),
            modelName,
          ...
          if (fs.existsSync(demucsOutputDir)) {
            ...
            logger.info(
              `Separation completed successfully for trackId: ${trackId}, jobId: ${jobId}`
            );
          } else {
            throw new Error(
              `Demucs output directory not found: ${demucsOutputDir}`
            );
          }
        } else {
        ...
        logger.error(
          `Error in separation completion handler: ${error.message}`
        );
