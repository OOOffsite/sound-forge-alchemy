...
async function getInstalledModels() {
  try {
    ...
    const modelMatch = stdout.match(/--model (\w+) *Model to use, one of: ([^\n]+)/);
    ...
    return new Promise((resolve, reject) => {
      pythonProcess.on('close', (code) => {
        if (code === 0) {
          ...
          console.error(`Error downloading model: ${modelName}, exit code: ${code}`);
...
async function getDefaultModel() {
  try {
    ...
    const installedDefaultModel = allModels.find(model => model.isDefault && model.installed);
