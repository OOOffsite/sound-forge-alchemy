...
def analyze_track(audio_path, track_id, job_id, redis_url):
    ...
    try:
        ...
        # Use librosa to get sample rate and duration (works for MP3 and all formats)
        try:
            # Get duration and sample rate using librosa
            duration = librosa.get_duration(filename=audio_path)
            sr = librosa.get_samplerate(audio_path)
            print(f"Sample rate: {sr}, Duration: {duration:.2f} seconds")
        except Exception as info_exc:
            print(f"Could not get audio info with librosa: {info_exc}. Will estimate after loading.")
            duration = None
            sr = None
        ...
        if file_size > 50 * 1024 * 1024:  # If file is larger than 50MB
            target_sr = min(target_sr, 22050)  # Use lower sample rate for large files
            print(f"Large file detected, downsampling to {target_sr} Hz")
        
        # Load audio with memory efficiency in mind (always use librosa)
        try:
            # For larger files, load directly as mono at potentially lower sample rate
            if file_size > 10 * 1024 * 1024:  # 10MB
                print("Loading audio as mono to save memory (librosa)")
                y, sr = librosa.load(audio_path, sr=target_sr, mono=True, res_type='kaiser_fast')
                y = np.reshape(y, (1, -1))
            else:
                # For smaller files, load stereo if possible
                y, sr = librosa.load(audio_path, sr=target_sr, mono=False, res_type='kaiser_fast')
                if y.ndim == 1:
                    y = np.reshape(y, (1, -1))
            if duration is None:
                duration = y.shape[1] / sr
        except Exception as load_exc:
            print(f"Audio loading failed: {load_exc}. Trying fallback (mono, short duration, librosa)")
            y, sr = librosa.load(audio_path, sr=target_sr, mono=True, res_type='kaiser_fast', duration=300)
            y = np.reshape(y, (1, -1))
            if duration is None:
                duration = y.shape[1] / sr
